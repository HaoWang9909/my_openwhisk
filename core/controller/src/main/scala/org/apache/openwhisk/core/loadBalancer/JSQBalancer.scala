...
case class JSQBalancerConfig(usePerMinThrottle: Boolean)

// 修改最少连接数选择器类
private class LeastConnectionsInvokerSelector(loadBalancer: JSQBalancer)(implicit logging: Logging) {
  def selectInvoker(invokers: IndexedSeq[InvokerHealth])(implicit ec: ExecutionContext): Future[Option[InvokerInstanceId]] = {
    if (invokers.isEmpty) {
      ...
      if (healthyInvokers.isEmpty) {
        ...
        // 获取所有健康invoker的当前活跃请求数
        val invokersWithCountFutures = healthyInvokers.map { invoker =>
          // 确保使用正确的invoker ID格式
          val invokerId = invoker.id.instance.toString
          loadBalancer.activeActivationsByInvoker(invokerId).map { count =>
            logging.info(this, s"Invoker $invokerId has $count active requests")
            (invoker.id, count)
          }
        }
        
        Future.sequence(invokersWithCountFutures).map { invokersWithCount =>
          // 添加日志以便调试
          invokersWithCount.foreach { case (id, count) =>
            logging.info(this, s"Invoker ${id.instance} has $count active requests")
          }
          
          // 选择活跃请求数最少的invoker
          val (selectedInvoker, activeCount) = invokersWithCount.minBy(_._2)
          logging.info(this, s"Selected invoker ${selectedInvoker.instance} with $activeCount active requests")
          Some(selectedInvoker)
        }
      }
    }
  }
...
class JSQBalancer(config: WhiskConfig,
  ...
  private val invokerSelector = new LeastConnectionsInvokerSelector(this)(logging)
  ...
  override def publish(action: ExecutableWhiskActionMetaData, msg: ActivationMessage)(
    ...
    // 使用最少连接数选择器选择invoker
    val selectedInvokerFuture = invokerHealth().flatMap { invokers =>
      logging.info(this, s"Available invokers: ${invokers.map(inv => s"${inv.id.instance}:${inv.status}").mkString(", ")}")
      invokerSelector.selectInvoker(invokers)
    }
    ...
    selectedInvokerFuture.flatMap {
      ...
      case Some(selectedInvoker) =>
        logging.info(this, s"Publishing activation ${msg.activationId} to invoker ${selectedInvoker.instance}")
                  ...
                  val schedulerFuture = res.getKvsList.asScala.headOption match {
                    case Some(kv) =>
                      ...
                      SchedulerEndpoints
                        ...
                        .getOrElse {
                          JSQBalancer.schedule(schedulerEndpoints.values.toIndexedSeq)
                            .map(createNewQueue)
  ...
  private def setupActivation(msg: ActivationMessage,
    ...
    // 确保正确更新invoker的活跃请求计数
    val invokerId = Option(msg.rootControllerIndex).map(id => 
      InvokerInstanceId(id.asString.toInt, userMemory = 0.MB))
    invokerId.foreach { id =>
      logging.debug(this, s"Incrementing active count for invoker ${id.instance}")
  ...
  override def activeActivationsByInvoker(invoker: String): Future[Int] = {
    logging.debug(this, s"Getting active activations for invoker $invoker")
    val invokerId = InvokerInstanceId(invoker.toInt, userMemory = 0.MB)
    val count = activationsPerInvoker.get(invokerId).map(_.intValue()).getOrElse(0)
    logging.debug(this, s"Invoker $invoker has $count active activations")
...
object JSQBalancer extends LoadBalancerProvider {
  ...
  override def instance(whiskConfig: WhiskConfig, instance: ControllerInstanceId)(implicit actorSystem: ActorSystem,
    ...
    new JSQBalancer(whiskConfig, instance, etcd, feedFactory)
  }
