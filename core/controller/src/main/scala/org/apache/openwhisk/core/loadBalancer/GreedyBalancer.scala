...
case class GreedyBalanceConfig(usePerMinThrottle: Boolean)


private class GreedyInvokerSelector {
  // 为每个Invoker创建一个NestedSemaphore来跟踪资源使用
  private val invokerSemaphores = TrieMap[InvokerInstanceId, NestedSemaphore[String]]()
  
  def selectInvoker(invokers: IndexedSeq[InvokerHealth], action: ExecutableWhiskActionMetaData)(implicit logging: Logging, transid: TransactionId): Option[InvokerInstanceId] = {
    if (invokers.isEmpty) {
      logging.info(this, s"No available invoker")
      None
    } else {
      val healthyInvokers = invokers.filter(_.status == Healthy)
      if (healthyInvokers.isEmpty) {
        logging.info(this, s"There is no healthy invoker")
        None
      } else {
        // 找到负载最小的健康Invoker
        val selectedInvoker = healthyInvokers.maxBy { invoker =>
          val semaphore = invokerSemaphores.getOrElseUpdate(
            invoker.id,
            new NestedSemaphore[String](invoker.id.userMemory.toMB.toInt))
          semaphore.availablePermits
        }
        
        logging.info(this, s"chose invoker ${selectedInvoker.id}, Current available resources: ${invokerSemaphores(selectedInvoker.id).availablePermits}")
        
        val semaphore = invokerSemaphores(selectedInvoker.id)
        
        // 尝试获取资源
        if (semaphore.tryAcquireConcurrent(
          action.fullyQualifiedName(true).toString,
          action.limits.concurrency.maxConcurrent,
          action.limits.memory.megabytes)) {
          logging.info(this, s"Successfully allocate resources for action ${action.fullyQualifiedName(true)} on invoker ${selectedInvoker.id}")
          Some(selectedInvoker.id)
        } else {
          logging.warn(this, s"Invoker ${selectedInvoker.id} resource allocation failed, try other invoker")
          // 如果获取失败,尝试其他Invoker
          val remainingInvokers = healthyInvokers.filter(_.id != selectedInvoker.id)
          if (remainingInvokers.isEmpty) {
            logging.error(this, s"No other invokers are available.")
            None
          } else {
            selectInvoker(remainingInvokers, action)
          }
        }
      }
    }
  ...
  def releaseInvoker(invokerId: InvokerInstanceId, action: ExecutableWhiskActionMetaData)(implicit logging: Logging, transid: TransactionId): Unit = {
    invokerSemaphores.get(invokerId).foreach { semaphore =>
      semaphore.releaseConcurrent(
        action.fullyQualifiedName(true).toString,
        action.limits.concurrency.maxConcurrent,
        action.limits.memory.megabytes)
      logging.info(this, s"Release resources for action ${action.fullyQualifiedName(true)} on invoker ${invokerId}")
    }
  }
}

class GreedyBalancer(config: WhiskConfig,
  ...
  private val invokerSelector = new GreedyInvokerSelector()
  ...
  override def publish(action: ExecutableWhiskActionMetaData, msg: ActivationMessage)(
    ...
    val selectedInvokerFuture = invokerHealth().map { invokers =>
      invokerSelector.selectInvoker(invokers, action)
    }

    selectedInvokerFuture.flatMap {
      case None =>
        logging.error(
          this,
          s"Failed to invoke action ${action.fullyQualifiedName(false)}, error: no healthy invoker available")
        Future.failed(LoadBalancerException("No healthy invoker available"))
        
      case Some(selectedInvoker) =>
        if (schedulerEndpoints.isEmpty) {
          logging.error(
            this,
          ...
          val modifiedMsg = msg.copy(rootControllerIndex = ControllerInstanceId(selectedInvoker.instance.toString))

          def processEndpoint(endPoint: SchedulerEndpoints): Future[String] = {
            Future.successful(
              schedulerEndpoints.getOrElse(endPoint, Random.shuffle(schedulerEndpoints.toList).head._2).sid.toString)
          }

          def createNewQueue(scheduler: SchedulerStates): Future[String] = {
            createQueue(invocationNamespace, action.toWhiskAction, msg.action, msg.revision, scheduler)
            Future.successful(scheduler.sid.toString)
          }

          val baseNameFuture = queueEndpoints.get(key) match {
            case Some(endPoint) => processEndpoint(endPoint)
            case None =>
              etcdClient
                .get(key)
                .flatMap { res =>
                  val schedulerFuture = res.getKvsList.asScala.headOption match {
                    case Some(kv) =>
                      ...
                      SchedulerEndpoints
                        ...
                        .getOrElse {
                          GreedyBalancer.schedule(schedulerEndpoints.values.toIndexedSeq)
                            .map(createNewQueue)
                            .getOrElse(Future.failed(LoadBalancerException("No scheduler available")))
                        }
                    case None =>
                      GreedyBalancer.schedule(schedulerEndpoints.values.toIndexedSeq)
                        .map(createNewQueue)
                ...
                .recoverWith {
                  case _ => Future.failed(LoadBalancerException("No scheduler endpoint available"))
                }
          }

          baseNameFuture.flatMap { baseName =>
            val topicName = Controller.topicPrefix + baseName
            val activationResult = setupActivation(modifiedMsg, action)
            
            // 在完成时释放资源
            activationResult.onComplete { _ =>
              invokerSelector.releaseInvoker(selectedInvoker, action)
            }
            
            sendActivationToKafka(messageProducer, modifiedMsg, topicName).map(_ => activationResult)
          }
        }
    }
  }
...
object GreedyBalancer extends LoadBalancerProvider {
  ...
  override def instance(whiskConfig: WhiskConfig, instance: ControllerInstanceId)(implicit actorSystem: ActorSystem,
    ...
    new GreedyBalancer(whiskConfig, instance, etcd, feedFactory)
  }
