...
case class RoundRobinConfig(usePerMinThrottle: Boolean)

// 添加RoundRobin选择器类
private class RoundRobinInvokerSelector {
  private val currentInvokerIndex = new AtomicInteger(0)
  
  def selectInvoker(invokers: IndexedSeq[InvokerHealth]): Option[InvokerInstanceId] = {
    if (invokers.isEmpty) {
      None
    } else {
      ...
      if (healthyInvokers.isEmpty) {
        ...
        val index = currentInvokerIndex.getAndIncrement() % healthyInvokers.length
        Some(healthyInvokers(index).id)
      }
    }
  }
}

class RoundRobinBalancer(config: WhiskConfig,
  ...
  private val invokerSelector = new RoundRobinInvokerSelector()
    ...
    selectedInvokerFuture.flatMap {
      ...
      case Some(selectedInvoker) =>
        if (schedulerEndpoints.isEmpty) {
          ...
          // 修改消息，使用invoker的instance作为controller index
                  ...
                  val schedulerFuture = res.getKvsList.asScala.headOption match {
                    case Some(kv) =>
                      ...
                      SchedulerEndpoints
                        ...
                        .getOrElse {
                          RoundRobinBalancer.schedule(schedulerEndpoints.values.toIndexedSeq)
                            .map(createNewQueue)
...
object RoundRobinBalancer extends LoadBalancerProvider {
  ...
  override def instance(whiskConfig: WhiskConfig, instance: ControllerInstanceId)(implicit actorSystem: ActorSystem,
    ...
    new RoundRobinBalancer(whiskConfig, instance, etcd, feedFactory)
  }
