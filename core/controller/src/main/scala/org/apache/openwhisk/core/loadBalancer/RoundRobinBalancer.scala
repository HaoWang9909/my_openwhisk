...
case class RoundRobinBalancerConfig(usePerMinThrottle: Boolean)
...
class RoundRobinBalancer(config: WhiskConfig,
  ...
  private val activationsPerNamespace = TrieMap[org.apache.openwhisk.core.entity.UUID, LongAdder]()
  ...
  override def publish(action: ExecutableWhiskActionMetaData, msg: ActivationMessage)(
    ...
    invokerHealth().flatMap { invokers =>
      // 使用轮询方式选择一个invoker
      RoundRobinBalancer.selectInvoker(invokers) match {
        case Some(invoker) =>
          // 使用选中的invoker创建消息
          val activationMessage = msg.copy(rootControllerIndex = ControllerInstanceId(invoker.instance.toString))
          ...
          if (schedulerEndpoints.isEmpty) {
            ...
            // 使用轮询方式选择调度器
            val selectedScheduler = RoundRobinBalancer.schedule(schedulerEndpoints.values.toIndexedSeq)
            selectedScheduler match {
              case Some(scheduler) =>
                // 创建队列并返回调度器ID
                createQueue(invocationNamespace, action.toWhiskAction, msg.action, msg.revision, scheduler)
                val topicBaseName = scheduler.sid.toString
                val topicName = Controller.topicPrefix + topicBaseName
                val activationResult = setupActivation(activationMessage, action)
                sendActivationToKafka(messageProducer, activationMessage, topicName).map(_ => activationResult)
              case None =>
                Future.failed(LoadBalancerException("No scheduler endpoint available"))
            }
          }
...
object RoundRobinBalancer extends LoadBalancerProvider {
  // 将roundRobinCounter移到companion object中
  private val roundRobinCounter = new AtomicInteger(0)
   ...
   * 使用轮询方式选择调度器和invoker
   *
   * @param schedulers 可用的调度器列表
   * @param excludeSchedulers 需要排除的调度器
   * @return 选中的调度器
   */
  ...
  def selectInvoker(invokers: IndexedSeq[InvokerHealth]): Option[InvokerInstanceId] = {
    if (invokers.isEmpty) {
      ...
      if (healthyInvokers.isEmpty) {
        ...
        // 获取当前计数器值并递增
        val currentCounter = roundRobinCounter.getAndIncrement()
        
        // 使用取模运算选择invoker
        val selectedIndex = Math.abs(currentCounter % healthyInvokers.length)
        Some(healthyInvokers(selectedIndex).id)
      }
    }
  }
}
